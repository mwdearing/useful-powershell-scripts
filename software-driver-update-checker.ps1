...
If your environment enforces signed scripts, run this tool through the companion wrapper: `run-software-driver-update-checker.cmd`.
...
function Test-ChannelFailure {
    ...
    if (($Result.PSObject.Properties.Name -contains 'Checked') -and -not $Result.Checked -and $Result.Available -eq $false) {
        return $true
    }
